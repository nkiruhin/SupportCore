...
var sipClientStart = function sipClientStart(url) {
...
var signalRStart = function signalRStart() {
    console.log("signalr client start");
    var connection = new signalR.HubConnectionBuilder().withUrl("/hub").build();
    connection.on("ReceiveMessage", function (user, date, message, isuser) {
...
//Error Notify
var onError = function onError(context) {
...
var loadTicketStat = function loadTicketStat(url) {
    $.ajax({
        type: "POST",
        url: url,
...
var OnSuccessChart = function OnSuccessChart(response) {
...
var loadTicketStatforStaff = function loadTicketStatforStaff(url) {
    $.ajax({
        type: "POST",
        url: url,
        contentType: "application/json; charset=utf-8",
...
var OnSuccessChartforStaff = function OnSuccessChartforStaff(response) {
    var data = response;
    var options = {
...
$(document).ajaxError(function (e, xhr) {
    if (xhr.status === 401) window.location = "/Account/Login";else if (xhr.status === 403) alert("You have no enough permissions to request this resource.");
});

// For ajax data return
// Get FildValue for Slays
var GetFieldValue = function GetFieldValue(url, field) {
    var option = '';
    $.getJSON(url, function (data) {
...
var enableSelect = function enableSelect() {
    // Enable select2 on ticket forms for nonUser
...
var disInput = function disInput() {
...
var addEvent = function addEvent(message, user, date) {
    var bell = $('.eventAlarm');
...
var getTemplate = function getTemplate(editor, url) {
